# Runtime Governance Specification Conformance

Maps this framework's implementation to the agentic runtime governance specification
(Core requirements R1-R6 all MUST; Extended requirements R1-R9 add R7-R9 SHOULD).

Target level: **Extended** (technical requirements R1-R9).

> Scope note: this documents the *technical* requirements. Public conformance claims
> also require organizational conditions (production customers, security certification,
> external review). Those are not code and are tracked separately; this document does
> not assert a public conformance claim.
...
This deployment uses a **Vendor Integration** pattern (Bedrock Agent action-group entry)
plus a control-plane pipeline. The decision engine is model- and runtime-agnostic;
the Bedrock-specific bindings (guardrail, action-group entry) are the integration surface.
...
- R9 / role separation: `tests/test_compromised_enforcer.py::TestRoleSeparation`
  ...
  does not, via direct IAM policy inspection against live infrastructure
  (requires `LIVE_AWS=1`).
