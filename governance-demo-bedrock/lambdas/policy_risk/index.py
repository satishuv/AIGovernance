...
def handler(event, context):
    ...
    risk_assessment = risk_engine.compute_risk(action_request)
    final_risk = min(100, risk_assessment.risk_score + risk_score_adjustment)

    # 4. Decision
    decision_engine = DecisionEngine()
    verdict = decision_engine.decide(policy_result, final_risk)

    decision_id = str(uuid.uuid4())

    explanation = (
        f"Action '{action_group}' {verdict} by policy '{policy_result.policy_id}'. "
        f"Risk score ({final_risk}) "
    )
    if verdict == "allow":
        explanation += "is below the escalation threshold."
    elif verdict == "deny":
        explanation += "is not considered when policy explicitly denies."
    elif verdict == "escalate":
        explanation += "exceeds escalation threshold."

    decision = {
        "decision_id": decision_id,
        "agent_id": agent_id,
        "action_requested": action_group,
        "verdict": verdict,
